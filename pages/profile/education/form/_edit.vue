    ...
    <v-container v-else fluid>
      <validation-observer ref="observer">
        <form @submit.prevent="submit">
          ...
          <v-btn
            v-if="educationHistory.docpath"
            color="primary"
            class="mb-5"
            :href="`http://hris.bosowa.co.id/rest_api_jova/file/educationdocument?filename=${educationHistory.docpath}`"
            target="_blank"
            small
          >
            Lihat file
          ...
          <v-btn color="red" dark @click="deleteEducationHistory">
  ...
  methods: {
    async getEducationHistory() {
      const response = await this.$axios.get(
        '/profile/education?id=' + this.$route.params.edit
      )
      this.educationHistory = response.data
    },
    async deleteEducationHistory() {
      if (confirm('Hapus ' + this.educationHistory.edu_name) === true) {
