<template>
  <v-card>
    <v-card-title>Ubah keahlian berbahasa</v-card-title>
    <v-divider />
    ...
    <v-container v-else fluid>
      <validation-observer ref="observer">
        <form @submit.prevent="submit">
          <v-text-field
            v-model="languageSkill.language_name"
            readonly
            dense
            label="Bahasa"
            outlined
          ...
          <v-btn color="red" dark @click="deleteLanguageSkill">
...
export default {
  ...
  async fetch() {
    ...
    await this.getLanguageSkill()
  },
  methods: {
    async getLanguageSkill() {
      const response = await this.$axios.get(
        '/skillinterest/language?id=' + this.$route.params.edit
      )
      this.languageSkill = response.data
    },
    async deleteLanguageSkill() {
      if (confirm(`Hapus ${this.languageSkill.language_name}`) === true) {
        await this.$axios.delete(
          `/skillinterest/language?id=${this.languageSkill.applanguage_id}`
        )
        this.$router.back()
      }
    },
    async submit() {
      if (await this.$refs.observer.validate()) {
        try {
          this.loading = true
          await this.$axios.put('/skillinterest/language', this.languageSkill)
          this.$router.back()
        } catch (error) {
          alert(error)
        } finally {
