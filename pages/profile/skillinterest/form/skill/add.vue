<template>
  <v-card>
    <v-card-title>Tambah keahlian</v-card-title>
    <v-divider />
    ...
    <v-container v-else fluid>
      <validation-observer ref="observer">
        <form @submit.prevent="submit">
          ...
          >
            <v-select
              v-model="skill.skill_code"
              :items="$store.getters['masterType/skills']"
            ...
            >
              <template #label>
                Keahlian <span class="red--text">*</span>
              </template>
            </v-select>
          </validation-provider>
          <validation-provider
  ...
  methods: {
    async submit() {
      if (await this.$refs.observer.validate()) {
        try {
          this.loading = true
          await this.$axios.post('/skillinterest/skill', this.skill)
          this.$router.back()
        } catch (error) {
          alert(error)
        } finally {
